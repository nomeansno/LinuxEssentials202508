...
## Übung 7: Position der Option -t

Erkläre, warum folgendes Kommando funktionert:
```bash
tar -tf archiv.tar
tar -tvf archiv.tar
```

Und warum folgende Kommdos **nicht** funktionieren:
```bash
tar -ft archiv.tar
tar -ftv archiv.tar
tar -vft archiv.tar
...
Teste nun noch folgendes Kommando:
```bash
tar ft archiv.tar
```
